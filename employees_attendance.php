...
if (!canAccess('employees_attendance')) {
...
function syncEmployeesAttendanceArchive($conn, $employees, $weekDays, $todayDate)
{
    ...
    foreach ($employees as $employee) {
        ...
        $selectStmt->execute([(int) $employee['id'], $createdDate, $todayDate]);
...
try {
    ...
    if (!$barcodeColumnStmt->fetchColumn()) {
        try {
            $conn->exec("ALTER TABLE employees ADD COLUMN employee_barcode VARCHAR(100) NOT NULL DEFAULT ''");
    ...
    die("تعذر تجهيز صفحة حضور الموظفين");
}
...
$employeesStmt = $conn->prepare(
    "SELECT id, employee_name, employee_number, employee_barcode, attendance_time, departure_time, off_days, created_at
     FROM employees
...
syncEmployeesAttendanceArchive($conn, $employees, $weekDays, $todayDate);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($scanValue === '') {
        ...
        try {
            ...
            $employeeLookupStmt = $conn->prepare(
                "SELECT id, employee_name, employee_number, employee_barcode, attendance_time, departure_time, off_days
                 FROM employees
                 WHERE employee_barcode = ? OR employee_number = ?
                 ORDER BY id ASC
                 LIMIT 2"
            );
            $employeeLookupStmt->execute([$scanValue, $scanValue]);
            $matchedEmployees = $employeeLookupStmt->fetchAll(PDO::FETCH_ASSOC);

            if (count($matchedEmployees) === 0) {
                throw new RuntimeException('الباركود غير مسجل');
            }

            if (count($matchedEmployees) > 1) {
                throw new RuntimeException('الباركود مرتبط بأكثر من موظف');
            }
            ...
            $scheduledAttendance = parseAttendanceDateTime($todayDate, $employee['attendance_time']);
            $scheduledDeparture = parseAttendanceDateTime($todayDate, $employee['departure_time']);

            if ($scheduledAttendance === null || $scheduledDeparture === null) {
                throw new RuntimeException('مواعيد الموظف غير صالحة');
            }
            ...
            $recordStmt->execute([(int) $employee['id'], $todayDate]);
            ...
            if (empty($record['check_in_at'])) {
                ...
                $successMessage = 'تم تسجيل الحضور لـ ' . $employee['employee_name'];
                ...
                $successMessage = 'تم تسجيل الانصراف لـ ' . $employee['employee_name'];
            } else {
                throw new RuntimeException('تم تسجيل حضور وانصراف الموظف اليوم بالفعل');
            }

            $conn->commit();
            header('Location: employees_attendance.php?success=' . urlencode($successMessage));
            exit;
...
$recordsStmt = $conn->prepare(
    "SELECT
        ...
        employees_attendance.record_date,
        employees_attendance.scheduled_attendance_time,
        employees_attendance.scheduled_departure_time,
        employees_attendance.check_in_at,
        employees_attendance.check_out_at,
        employees_attendance.attendance_status,
        employees_attendance.departure_status,
        employees_attendance.day_status,
        employees_attendance.is_off_day,
        employees.employee_name,
        employees.employee_number,
        employees.employee_barcode
     FROM employees_attendance
     INNER JOIN employees ON employees.id = employees_attendance.employee_id
     ORDER BY employees_attendance.record_date DESC, employees_attendance.id DESC"
);
$recordsStmt->execute();
        ...
        <main class="main-content">
            ...
            <div class="content-card attendance-content-card">
                <div class="page-header attendance-page-header">
                    <div>
                        <h1 class="section-title">📅 حضور الموظفين</h1>
                ...
                <div class="barbers-overview attendance-overview">
                    <div class="overview-card">
                        ...
                        <strong class="overview-value"><?php echo $totalEmployees; ?></strong>
                    </div>
                    <div class="overview-card">
